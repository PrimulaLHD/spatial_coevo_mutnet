...
h2c <-
    aaply(1:length(p_A), 1, function(i)
    {
        aaply(1:100, 1, function(j)
        {
            
            init_A <- rnorm(n_sp, 20, sqrt(10))
            init_B <- rnorm(n_sp, 40, sqrt(10))
           ...
           c(meanA, meanB, wmeanA, wmeanB, wm_fin_A, wm_fin_B, varA, varB) 
    })}, .parallel = TRUE)
...
```{r h2c_hand}
h2c.df <- adply(h2c, 1)
colnames(h2c.df) <- c('hotness', 'meanA', 'meanB', 'wmeanA', 'wmeanB', 'wm_fin_A',
                      'wm_fin_B', 'varA', 'varB') 

h2c.df $ hotness <- rep(p_A, each = 100)

h2c.df %>%
    group_by(hotness) %>%
    summarise_each(funs(mean)) %>%
    ggplot(.) +
    geom_line(aes(x = hotness, y = meanA), color = 'red') +
    geom_line(aes(x = hotness, y = meanB), color = 'blue')
